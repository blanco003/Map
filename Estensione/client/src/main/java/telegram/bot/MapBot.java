...
public class MapBot extends TelegramLongPollingBot {
    ...
    private void gestisci_input(Utente utente, String message_text) throws IOException, ClassNotFoundException{
        ...
        if (utente.getStato().equals("carica_dati")) {
            ...
            // se l'utente ha scelto di eliminare un dataset dal db allora spediamo al server un 2 
            utente.getConnessione().getObjectOutputStream().writeObject(2);

            controlla_profondita(utente, message_text);

            
        } else if(utente.getStato().equals("controlla_profondita")){

            /* l'utente aveva gia eseguito il comando per inserire la profondita dunque il server è ancora in attesa che l'utente
    ...
    private void gestisci_chiamata_di_ritorno(Update update) throws IOException{
        ...
        if(utente.getStato().equals("attesa_risposta")){ 
            ...
            //TODO: se l'utente si trova nello stato attesa_risposta puo comunque rispondere a vecchi messaggi con questa versione
     ...
     * @param message_text Messaggio inviato dall'utente
     */
    public void controlla_profondita(Utente utente,String message_text){
    ...
    public String getBotUsername() {
        return "map_progetto_bot";
    }

    // ereditato da TelegramLongPollingBot
    @Override
    public String getBotToken() {
        return "7172711483:AAE4-J_ZzYQV5CS6SKeFC0xSHV7rwBnJOZY";
    }
